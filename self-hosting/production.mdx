...
TryPost uses Laravel's task scheduler for recurring jobs like dispatching scheduled posts and cleaning up expired tokens. The scheduler **must** run every minute or scheduled posts won't be published on time.
...
<Warning>
  This is required. Without the cron job, scheduled posts will not be published automatically.
</Warning>
